import time, unittest, sqlalchemy, numpy, datetime, logging, sys
...
class TestService(unittest.TestCase):
    ...
    def check_campaign(self, license, party, datastream, multi_datastream, observation_group, campaign):
        ...
        logger.debug(f"datastream: {json.dumps(transform_entity_to_json_dict(datastream))}")
        logger.debug(f"datastreams.entities[0]: {json.dumps(transform_entity_to_json_dict(datastreams.entities[0]))}")
    ...
    def check_location(self, thing, location):
        ...
        logger.debug(f"location: {json.dumps(transform_entity_to_json_dict(location))}")
        logger.debug(f"locations.entities[0]: {json.dumps(transform_entity_to_json_dict(locations.entities[0]))}")
