    ...
    if isinstance(value, str):
        if '/' in value:
            try:
                times = value.split('/')
                if len(times) != 2:
                    raise ValueError("If the datetime interval is provided as a string,"
                                     " it should be in isoformat")
                result = [datetime.fromisoformat(times[0]),
                          datetime.fromisoformat(times[1])]
            except ValueError:
                raise ValueError("If the datetime entity interval is provided as a string,"
                                 " it should be in isoformat")
            ...
            try:
                ...
                raise ValueError("If the datetime is provided as string, it should be in isoformat")
